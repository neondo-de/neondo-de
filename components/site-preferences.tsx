...
export default function SitePreferences(){
  ...
  useEffect(()=>{
    ...
    if(savedLang && labels[savedLang]) setLang(savedLang);
  },[]);
  ...
  return <div className="neondo-preferences" aria-label="Site preferences">
    ...
    <div className="neondo-language">
      <button className="neondo-language-trigger" onClick={()=>setOpen(v=>!v)} aria-expanded={open}><span>DE</span><b>{lang}</b><i>⌄</i></button>
      {open&&<div className="neondo-language-menu">{(Object.keys(labels) as Lang[]).map(code=><button key={code} className={code===lang?"active":""} onClick={()=>choose(code)}><span>{code}</span>{labels[code]}</button>)}</div>}
    </div>
  </div>
}
